...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('arbori_clothes_civil'), value = 'citizen_wear'},
				{label = _U('arbori_clothes_work'), value = 'arbori_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'arbori_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'arbori_actions_menu'
...
function OpenArboriActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'arbori_actions',
		{
			css      = 'entreprise',
			title    = _U('arbori'),
			align    = 'top-left',
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'arboriculturist', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'arboriculturist', vehicleProps)
		...
		local elements = {
			{label = _U('veh_work'),  value = 'sadler'},
		}
...
function OpenMobileArboriActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_arbori_actions',
		{
			css      = 'job',
			title    = _U('arbori_actions'),
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_arboriculturist', 'Arboriculturist', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_arboriculturistjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_arboriculturistjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_arboriculturistjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_arboriculturistjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_arboriculturistjob:hasEnteredMarker', function(zone)
	if zone == 'AppleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist'  then
		CurrentAction     = 'apple_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementCidre' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist'  then
		CurrentAction     = 'apple_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'ArboriActions' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
		CurrentAction     = 'arbori_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
...
AddEventHandler('esx_arboriculturistjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'AppleFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
		TriggerServerEvent('esx_arboriculturistjob:stopHarvest')
	end  
	if (zone == 'TraitementCidre') and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
		TriggerServerEvent('esx_arboriculturistjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
		TriggerServerEvent('esx_arboriculturistjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.ArboriActions.Pos.x, Config.Zones.ArboriActions.Pos.y, Config.Zones.ArboriActions.Pos.z)
		...
		AddTextComponentString(_U('arbori_cloakroom'))
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_arboriculturistjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_arboriculturistjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'apple_harvest' then
					TriggerServerEvent('esx_arboriculturistjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'apple_traitement' then
					TriggerServerEvent('esx_arboriculturistjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_arboriculturistjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'arbori_actions_menu' then
					OpenArboriActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'arboriculturist', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'arboriculturist' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileArboriActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)